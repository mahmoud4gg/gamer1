...
setInterval(() => {
  http.get(`https://gamer12.glitch.me`);
}, 280000);
...
const youtube = new YouTube("AIzaSyDV4aLuaKE3j51QmITlsx_x1QuG0lXI3_I"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDV4aLuaKE3j51QmITlsx_x1QuG0lXI3_I"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("ready", () => {
  ...
  client.user.setActivity(`${prefix}help`, { type: "👑YTcrep_king👑" });
});
  ...
  if (command == "say") {
    if (!message.member.hasPermission("ADMINISTRATOR"))
      return message.channel.send(
        "**ADMINISTRATOR انت مش  :rolling_eyes:**"
      );
...
client.on("message", message => {
  if (message.content === prefix + "بوت") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@724774444563693648>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  ...
  if (command == "بان") {
    ...
    if (!message.guild.member(message.author).hasPermission("BAN_MEMBERS"))
      return message.reply("**انت مو ادراي في سيرفر**");
    ...
    if (
      ...
      return message.channel.send("ما تقدر تبند شخص!");
    if (!message.guild.member(user).bannable)
      return message.reply(
        "**لازم رتبه بوت اعلي من رتبه شخص الي تبي تعطي بان**"
      );
...
client.on("message", message => {
  ...
  if (command == "طرد") {
    ...
    if (!message.guild.member(user).kickable)
      return message.reply(
        "**بوت ما يقدر يطرد رتبه اعلي مينو**"
      );
    if (
      message.mentions.members.first().highestRole.position >=
      message.member.highestRole.position
    )
      return message.channel.send("ما تقدر طرد لي شخص!");
...
client.on("message", message => {
  if (message.content.split(" ")[0] === prefix + "شكلي") {
...
client.on("message", message => {
  ...
  if (command === prefix + "احكي") {
    ...
    if (!muteRole)
      return message
        .reply("** لا يوجد لديك رتبه ميوت 'Muted' **")
        .catch(console.error);
    if (message.mentions.users.size < 1)
      return message
        .reply("** منشن شخص االاول**")
        .catch(console.error);
    const embed = new Discord.RichEmbed()
      .setColor(0x00ae86)
      .setTimestamp()
    ...
    if (message.guild.member(user).removeRole(muteRole.id)) {
      return message
        .reply("**:white_check_mark: ..  تم فك ميوت  **")    
        .catch(console.error);
    } else {
      message.guild
        .member(user)
...
client.on("message", message => {
  ...
  if (command === prefix + "اسكات") {
    ...
    if (!muteRole)
      return message
        .reply("** مافي  رتبه ميوت 'Muted' **")
        .catch(console.error);
    if (message.mentions.users.size < 1)
      return message
        .reply("**  منشنت شخص اولاً**")
        .catch(console.error);
    ...
    if (message.guild.member(user).roles.has(muteRole.id)) {
      return message
        .reply("**:white_check_mark: .. اخد عضو ميوت**")
        .catch(console.error);
    } else {
      message.guild
        .member(user)
...
client.on("message", message => {
  if (message.content === prefix + "قفل") {
    if (!message.channel.guild)
      return message.reply("  الامر فقط للسيرفرات !!");

    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.reply(" انت مو اداري ");
    message.channel
      .overwritePermissions(message.guild.id, {
        SEND_MESSAGES: false
      })
      .then(() => {
        message.reply("** قفل الشات :no_entry: **");
      });
  }
  if (message.content === prefix + "فتح") {
    ...
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.reply("انت مو ادري");
    message.channel
      .overwritePermissions(message.guild.id, {
        SEND_MESSAGES: true
      })
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "نقل")) {
    if (message.member.hasPermission("MOVE_MEMBERS")) {
      if (message.mentions.users.size === 0) {
        return message.channel.send("``Use : " + prefix + "نقل @User``");
      }
...
\`${prefix}بوت\` : لعرض معلومات عن البوت 
\`${prefix}user\` : لعرض معلومات عنك 
\`${prefix}شكلي\` :يعرض لك صورت  اي شخص عن طريق الايدي
\`${prefix}شكلي\` : لعرض صورتك أو صورة الي تمنشنه 
...
\`${prefix}بان\` : لحظر شخص من السيرفر
\`${prefix}كيك\` : لطرد شخص من السيرفر
\`${prefix}فتح\` : لفتح الشات
\`${prefix}قفل\` : لقفل الشات 
\`${prefix}اسكات\` : لإسكات شخص
\`${prefix}احكي\` : لـ فك إسكات شخص
\`${prefix}نقل\` : لسحب الشخص الى روومك
\`${prefix}setLog\` : لتحديد روم السجلات 
\`${prefix}role\` : لاعطاء شخص رتبة
\`${prefix}role all\` : لـ إعطاء الجميع رتبة معينة
...
`);
        ...
        .catch(() => {
          return message.channel
            .send(
              "**افتح خاص حقق عشان تجيك رساله ، ما اعرف ابعت لك رساله وي خاص حقق مقفول **"
            )
            .then(() => {
              return message.react("❌");
            });
...
client.on("message", message => {
  if (message.content === "انا زعلان") {
    message.channel.send("**لا تبكي يا صغير**");
...
client.on("message", message => {
  if (message.content === "هاي") {
    message.channel.send("**لا تقلد بنات**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "احمد") {
    message.channel.send("**الاسطوره وي المعلم**");
    message.channel.sendFile("");
  }
});

client.on("message", message => {
  if (message.content === "انا قوي") {
    message.channel.send("**النمله اقوه منك**");
    message.channel.sendFile("");
  }
});
...
client.on("message", message => {
  if (message.content === "كل زق") {
    message.channel.send("**ما في احد يبي ياكلك**");
    message.channel.sendFile("");
  }
});

client.on("message", message => {
  if (message.content === "بسوي سبام") {
    message.channel.send("**بتبلع ميوت**");
    message.channel.sendFile("");
  }
});

client.on("message", message => {
  if (message.content === "مين انت") {
    message.channel.send("**بوت حق محمود وي احمد**");
    message.channel.sendFile("");
  }
});

client.on("message", msg => {
  if (msg.content === "مساعده") { /// هنا تكتب الكلام
    msg.reply("**لدعم الفني في خدمتك 24ساعه نتشرف بي خدمتك السيرفر ده خاص بي الجيمر العربي بندعم فيه قنوات اليوتيوب والبثوث وكمان السيرفر في رتب العاب كثيره تقدر تاخذ رتبه من التقديم علي جميع رتب الجميرز **  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "هاشم") { /// هنا تكتب الكلام
    msg.reply("**ما راح يرد**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "HASHEM") { /// هنا تكتب الكلام
    msg.reply("**ما راح يرد**  "); /// هنا الرد

  }
});
client.on("message", msg => {
  if (msg.content === "بوت مين ده") { /// هنا تكتب الكلام
    msg.reply("**👑YTcrep_king👑**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "اهلين") { /// هنا تكتب الكلام
    msg.reply("**اهلين فيك منور**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "سلام") { /// هنا تكتب الكلام
    msg.reply("**سلام يا حب**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "باك") { /// هنا تكتب الكلام
    msg.reply("**ولكم باك منور**  "); /// هنا الرد
...
client.on("message", msg => {
  if (msg.content === "السلام عليكم") { /// هنا تكتب الكلام
    msg.reply("**وعليكم السلام ورحمة الله وبركاته**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "برب") { /// هنا تكتب الكلام
    msg.reply("**لا طول**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "مصر") { /// هنا تكتب الكلام
    msg.reply("**ام دونياه**  "); /// هنا الرد
...
client.on("message", msg => {
  if (msg.content === "هلا") { /// هنا تكتب الكلام
    msg.reply("**هلا بيك منور**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "اهلا") { /// هنا تكتب الكلام
    msg.reply("**هلا فيك منور**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "ابي رتبه") { /// هنا تكتب الكلام
    msg.reply("**بتبلع رتبه اسمه ميوت احلي رتبه لي عيونك**  "); /// هنا الرد

  }
});

client.on("message", msg => {
  if (msg.content === "فلسطين") { /// هنا تكتب الكلام
    msg.reply("**تاج راسك**  "); /// هنا الرد
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setby")) {
    ...
    if (
      ...
      return message.channel.send("**لست اداري . ❌**");
    message.channel.send(
...
client.on("message", message => {
  ...
  if (message.content.startsWith(prefix + "settings")) {
    if (message.author.id !== message.guild.owner.user.id)
      return message.channel.send(
        "**:closed_lock_with_key:  **فقط للأونر**"
      );
    ...
    if (message.content.startsWith(prefix + "settings limitskick")) {
      if (!num) return message.channel.send("**:1234: | أرسل رقم ! **");
      if (isNaN(num)) return message.channel.send("**:1234: | أرقام بس ! **");
  ...
  if (!anti[guild.id + entry.id]) {
    ...
    if (anti[guild.id + entry.id].actions >= config[guild.id].banLimit) {
      guild.members
        ...
        .ban()
        .catch(e =>
          guild.owner.send(`**⇏ | ${entry.username} حاول يطلع كل الي في سيرفر  **`)
        );
      anti[guild.id + entry.id].actions = 0;
      fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
        e
    ...
    if (!anti[member.guild.id + entry.id]) {
      ...
      if (
        ...
        member.guild.members
          ...
          .catch(e =>
            member.owner.send(
              `**⇏ | ${entry.username} حاول يطلع كل الي في سيرفر **`
            )
          );
        anti[member.guild.id + entry.id].actions = 0;
        fs.writeFile("./config.json", JSON.stringify(config, null, 2), function(
...
/////كود صنع رد تلقائي
client.on("message", async message => {
  if (message.content.startsWith(prefix + "ردد")) {
    ...
    message.channel.send(" |** اكتب رساله...** ").then(msg => {
      message.channel
        .awaitMessages(filter, {
          //R.I.P Royal Bot!
          maxMatches: 1,
        ...
        .then(collected => {
          ...
          msg.edit(":scroll: | اكتب رد... :pencil2: ");
...
client.on("message", async message => {
  ...
  let room = message.content.split(" ").slice(1);
  let findroom = message.guild.channels.find(r => r.name == room);
  if (message.content.startsWith(prefix + "قول رد")) {
    if (!welcome[message.guild.id]) {
      ...
      let embed = new Discord.RichEmbed()
        ...
        .addField(
          "Default Message:",
          `**Welcome [], You Joined by [inviter] invite**`
        )
        .setThumbnail(message.author.avatarURL)
        .setFooter(`${client.user.username}`);
      message.channel.sendEmbed(embed);
...
client.on("message", message => {
  if (message.content === prefix + "ممنوع") {
    var list_all = [];
...
client.on("message", message => {
  if (message.content.split(" ")[0] === prefix + "رابط") {
    ...
    const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setDescription(
        "** ارسلت رابط في خاص ، لو ما وصلك افتح خاص  **"
      )
      .setAuthor(client.user.username, client.user.avatarURL)
      .setAuthor(client.user.username, client.user.avatarURL)
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "setVc")) {
    ...
    if (!message.member.hasPermission("MANAGE_GUILD"))
      return message.channel.send(
        "**ADMINISTRATOR ما عندك :rolling_eyes: ** "
      );
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "ممنوع")) {
    if (!message.channel.guild) return;
    if (message.author.bot) return;
    let channel = message.guild.channels.find(gg => gg.name === "ممنوع");
    if (!channel)
      return message.reply(
...
client.on("message", message => {
  if (message.content.startsWith(prefix + "ممنوع")) {
    ...
    message.channel.send("**✅ تم انشاء روم **");
  }
});
...
client.on("message", message => {
  ...
  if (!message.guild) return;
  if (!msg.startsWith(prefix + "ممنوع")) return;
  if (!message.member.hasPermission("MANAGE_ROLES"))
...
client.on("message", async msg => {
  ...
  if (cmd === "play") {
    ...
    if (!queue) return msg.channel.send("on music playing to skip it");
    ...
    if (!queue) return msg.channel.send(":notes: on music paused to resume.");

    if (queue.playing)
      return msg.channel.send(":notes: on music paused to resume.");
    ...
    function npMsg(queue) {
      let m =
        !queue || !queue.songs[0] ? "on music playing." : "Now Playing...";
